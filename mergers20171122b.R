...
scatter_ba <- function(x){
  ## form the MEAN mean before and after
  ...
  ggplot(sample1_mean,
    ...
    ylab('') + ggtitle('Scatterplot for before and after MEAN') + scale_colour_manual(values=groupPalette)
...
sample1_mean <- dcast(sample1, TICKER + case ~ BA, value.var = 'STDEV', mean, na.rm = TRUE)
sample1_mean["sic"] <- NA
