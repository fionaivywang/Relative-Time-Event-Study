...
merger_part1 <- read_csv("0116US1.csv", 
                         col_types = cols(`Amendment Date` = col_date(format = "%Y/%m/%d"), 
                                          `Announce Date` = col_date(format = "%Y/%m/%d"), 
                                          `Completion/Termination Date` = col_date(format = "%Y/%m/%d")), 
                         na = "empty")

merger_part2 <- read_csv("0116US2.csv", 
                         col_types = cols(`Amendment Date` = col_date(format = "%Y/%m/%d"), 
                                          `Announce Date` = col_date(format = "%Y/%m/%d"), 
                                          `Completion/Termination Date` = col_date(format = "%Y/%m/%d")), 
                         na = "empty")
...
mydata <- merger[which(merger$DealStatus=="Completed"),]
...
mydata <- mydata[which(mydata$Targetcusip !="N.A."),]; mydata <- mydata[which(mydata$Targetcusip !=""),]
mydata <- mydata[which(mydata$Acquirercusip !="N.A."),]; mydata <- mydata[which(mydata$Acquirercusip !=""),]
...
# remove merger cases that both acquirer and target are the same company.
x <- mydata[which(mydata$Acquirercusip==mydata$Targetcusip),]
mydata <- mydata[which(!mydata$Acquirercusip==mydata$Targetcusip),]

# the number of cases that are not consistent in totalowned and deal attribute
x <- mydata[which((!mydata$TotalOwned==100)&(grepl("Company Takeover", mydata$DealAttributes))),]
...
cusip <- c(mydata$Acquirercusip,mydata$Targetcusip)
...
# import sic list
sic_list1 <- read_excel("~/Desktop/Paper idea/4-bloomberg/sic.xlsx")

sic_list2 <- melt(sic_list1,  id.vars="Group")
sic_list2 <- subset(sic_list2, select = -c(value))
colnames(sic_list2) <- c("asic2","tsic2")

# get the matrix of distribution of merger cases among sic2
# l <- as.data.frame(table(subset(mean_codet, select = c("asic2", "tsic2"))))
l <- mean_codet %>% count(asic2, tsic2)  
# l <- mean_codet %<>% count(asic2, tsic2)
# l <- aggregate(number ~ asic2 + tsic2, data = mean_codet, sum)
# count(mean_codet, c("asic2", "tsic2"))
# l <- count(groupBy(mean_codet, "cd_zero_bal"))

sic_list <- merge(sic_list2, l, all.x = TRUE)
sic_table <- dcast(sic_list, asic2 ~ tsic2, value.var = "n")
# save the sic2 matrix
write.csv(sic_table, file = "sic2_merger_case_frequency.csv")
# calculate the sum per column and per row
sic_table %>% add_count()
sic_table$colsum<-apply(sic_table[,2:84],1,sum,na.rm=T)
sic_table[84,]<-applym(sic_table,2,sum,na.rm=T)


# descriptive statistics
# general statistics
m <- summary(mean_codet)
write.csv(m, file = "summary.csv")
library(factoextra)
library(cluster)

desc_stats <- data.frame( Min=apply(mean_codet, 2, min, na.rm=T),#minimum
                          
                          Med=apply(mean_codet, 2, median, na.rm=T),#median
                          
                          Mean=apply(mean_codet, 2, mean, na.rm=T),#mean
                          
                          SD=apply(mean_codet, 2, sd, na.rm=T),#Standard deviation
                          
                          Max=apply(mean_codet, 2, max, na.rm=T)#maximum
                          
)

desc_stats <- desc_stats[6:10,]
desc_stats <- round(desc_stats, 3)#??????????????????3???head(desc_stats)
